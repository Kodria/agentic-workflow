...
function artifactNamesInRoot(root: string, reporter?: BundleDiagnosticReporter): Set<string> {
    ...
    for (const b of discoverAllBundles([root], reporter)) names.add(b.name);
    return names;
}

export function overrideStatus(contentRoot: string, earlierRoots: string[], reporter?: BundleDiagnosticReporter): OverrideStatus[] {
    const declared = Array.from(readRegistryManifest(contentRoot).overrides);
    if (declared.length === 0) return [];
    const earlier = new Set<string>();
    for (const root of earlierRoots) {
        for (const n of artifactNamesInRoot(root, reporter)) earlier.add(n);
    }
