...
function parseStructure(lines: string[], problems: string[]): ProcessSubgoal[] {
    ...
    for (const raw of lines) {
        ...
        if (sg) {
            ...
            if (text.length > MAX_ITEM_LENGTH) { problems.push(`"${sg[1]}" text must be at most ${MAX_ITEM_LENGTH} characters`); continue; }
            subgoals.push({ id: sg[1], text, operations: [] }); continue;
        }
        ...
        if (opText.length > MAX_ITEM_LENGTH) { problems.push(`"${op[1]}" text must be at most ${MAX_ITEM_LENGTH} characters`); continue; }
...
function parseRouting(lines: string[], problems: string[]): ProcessModelBody['routing'] {
    ...
    for (const [index, line] of tableLines.entries()) {
        ...
        if (cells.some((c) => c.length > MAX_ITEM_LENGTH)) { problems.push(`"Ruteo" row ${index} has a cell longer than ${MAX_ITEM_LENGTH} characters`); continue; }
...
export function parseProcessBody(source: string, file: string): ProcessParseResult<ProcessModelBody> {
    ...
    for (const [label, value] of [['Objetivo', objective], ['Cuándo aplica', appliesWhen], ['Terminación', termination]] as const) {
        ...
        else if (value.length > MAX_SECTION_LENGTH) problems.push(`"${label}" must be at most ${MAX_SECTION_LENGTH} characters`);
    }

    for (const [index, entry] of unverified.entries()) {
        if (entry.length > MAX_ITEM_LENGTH) problems.push(`"Sin verificar" entry ${index} must be at most ${MAX_ITEM_LENGTH} characters`);
    }
