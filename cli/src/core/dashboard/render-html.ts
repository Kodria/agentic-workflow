...
function privacyAndActions(snapshot: DashboardSnapshotV1): string {
    const actions = snapshot.sections.flatMap((section) => section.items.filter((item) => item.remediation).map((item) => `<li><span class="state ${item.state}" aria-label="${item.state}">${STATE_GLYPH[item.state]} ${STATE_TEXT[item.state]}</span><span>${escapeHtml(item.label)}</span><code>${escapeHtml(item.remediation!)}</code><button type="button" disabled aria-label="Copy command (static): ${escapeHtml(item.remediation!)}">Copy command (static)</button></li>`)).join('') || '<li class="empty">No remediation is required.</li>';
...
function projectHeaderActions(): string {
    return `<div class="project-header-actions" data-project-header-actions aria-label="Project actions"><button type="button" disabled aria-describedby="static-controls-note">Export evidence (static)</button><button type="button" disabled aria-describedby="static-controls-note">Attach evidence (static)</button></div>`;
}

function projectEvidenceComposition(): string {
    return `<div data-project-evidence aria-label="Project evidence composition"><div class="evidence-grid"><div><h3>Plans &amp; work</h3><div class="plan-card active" data-plan-card="active"><div><strong>Active plan</strong><span class="state attention">Not observed</span></div><p>Progress: no observation</p><button type="button" disabled aria-describedby="static-controls-note">View plan (static)</button></div><div class="plan-card blocked" data-plan-card="blocked"><div><strong>Blocked plan</strong><span class="state unavailable">Not observed</span></div><p>Progress: no observation</p><button type="button" disabled aria-describedby="static-controls-note">View plan (static)</button></div></div><div><h3>Impact &amp; traceability</h3><table class="evidence-table"><thead><tr><th scope="col">Type</th><th scope="col">Source</th><th scope="col">Status</th><th scope="col">Date</th></tr></thead><tbody><tr><td colspan="4" class="empty">No impact evidence is available in this snapshot.</td></tr></tbody></table></div></div><div class="closure-actions" data-closure-actions aria-label="Cycle closure actions"><div><h3>What remains to close the cycle</h3><p class="empty">A missing marker does not mean zero findings.</p></div><div><button type="button" disabled aria-describedby="static-controls-note">Complete QA (static)</button><button type="button" disabled aria-describedby="static-controls-note">Record retro (static)</button><button type="button" disabled aria-describedby="static-controls-note">Attach evidence (static)</button></div></div></div>`;
}
