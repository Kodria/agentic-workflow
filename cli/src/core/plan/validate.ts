...
function checkMarkdown(text: string, slice: PlanSlice): PlanValidationReport | undefined {
    ...
    if (text.slice(expectedHeading, expectedHeading + heading.length) !== heading) return diagnostic('PLAN_MARKDOWN_HEADING', 'slice heading must immediately follow anchor');
    ...
    for (const name of ['Surfaces', 'Implementation', 'Edge cases', 'Evidence', 'Fallback']) {
        ...
        if (!match) return diagnostic('PLAN_MARKDOWN_SECTION', 'slice section must contain prose');
    }
    return undefined;
}
...
export function validatePlanFile(planPath: string, cwd = process.cwd()): PlanValidationReport {
    ...
    let bytes: Buffer; try { if (fs.lstatSync(candidate).isSymbolicLink() || !fs.statSync(candidate).isFile()) return diagnostic('PLAN_PATH_UNSAFE', 'plan path must be a regular file'); if (fs.statSync(candidate).size > MAX_PLAN) return diagnostic('PLAN_LIMIT', 'plan exceeds maximum size'); bytes = fs.readFileSync(candidate); } catch { return diagnostic('PLAN_READ', 'plan cannot be read'); }
    if (bytes.includes(0xef) && bytes.toString('utf8').includes('\ufffd')) return diagnostic('PLAN_ENCODING', 'plan must be valid UTF-8');
    const text = bytes.toString('utf8'); const starts = count(text, START); const ends = count(text, END); const schemaMatch = /"schema"\s*:\s*"(compact-slices\/[^"\\\r\n]*)"/.exec(text); const signaled = starts > 0 || ends > 0 || /"schema"\s*:\s*"compact-slices\//.test(text);
    if (!signaled) return { state: 'legacy' };
    if (starts !== 1 || ends !== 1 || text.indexOf(START) > text.indexOf(END)) return schemaMatch && schemaMatch[1] !== 'compact-slices/v1' ? unsupported(schemaMatch[1]) : diagnostic('PLAN_MARKERS', 'compact markers must occur once in order');
