...
export function captureCycleEvidence(input: CaptureCycleEvidenceInput): CycleEvidenceV1 {
  ...
  const signatures = failures.map((item) => { const verdict = object(item, 'verdict'); if (typeof verdict.fingerprint !== 'string' || !verdict.fingerprint) throw new Error('verdict fingerprint is required'); return hash(`signature:${verdict.fingerprint}`); });
