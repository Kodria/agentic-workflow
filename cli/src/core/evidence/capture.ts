...
export function captureCycleEvidence(input: CaptureCycleEvidenceInput): CycleEvidenceV1 {
  ...
  if (typeof journal.journalId !== 'string' || !journal.journalId) throw new Error('journal identity is required');
  ...
  const failures = journal.verdicts.filter((item, index) => { const verdict = object(item, `verdict ${index}`); if (verdict.result !== 'pass' && verdict.result !== 'fail' && verdict.result !== 'inconclusive') throw new Error('verdict result is invalid'); iso(verdict.receivedAt, 'verdict receivedAt'); return verdict.result === 'fail'; });
  const signatures = failures.map((item, index) => hash(`qa:${index}:${iso(object(item, 'verdict').receivedAt, 'receivedAt')}`));
