...
export function captureCycleEvidence(input: CaptureCycleEvidenceInput): CycleEvidenceV1 {
  ...
  const adverseVerdictIds = new Set<string>();
  const adverseVerdicts = journal.verdicts.filter((item, index) => {
    const verdict = object(item, `verdict ${index}`);
    if (typeof verdict.id !== 'string' || !verdict.id) throw new Error('verdict id is invalid');
    if (verdictIds.has(verdict.id)) throw new Error('verdict id is duplicated');
    verdictIds.add(verdict.id);
    if (verdict.result !== 'pass' && verdict.result !== 'fail' && verdict.result !== 'inconclusive') throw new Error('verdict result is invalid');
    iso(verdict.receivedAt, 'verdict receivedAt');
    if (verdict.result !== 'pass') adverseVerdictIds.add(verdict.id);
  ...
  const fixes = journal.fixes.filter((item, index) => {
    ...
    if (typeof fix.closed !== 'boolean') throw new Error('fix closed is invalid');
    if (typeof fix.verdictId !== 'string' || !fix.verdictId || !adverseVerdictIds.has(fix.verdictId)) throw new Error('fix verdictId must reference an adverse verdict');
    return fix.closed;
  }).length;
