...
export interface DispatchRecord { id: string; taskId: string; at: string; routingAttemptId?: string; dispatchRequestId?: string; }
...
export function isWellFormedState(x: unknown): x is JournalState {
    ...
    if (!x.tasks.every(isWellFormedTask)) return false;
    ...
    // but cannot masquerade as this new acknowledged handoff.
    if ((x.planBinding as PlanBinding | undefined)?.schema === 'compact-slices/v2') {
        ...
        for (const dispatch of dispatches) {
            ...
            if (linked.has(dispatch.routingAttemptId)) return false;
            linked.add(dispatch.routingAttemptId);
            const attempt = attempts.find(candidate => candidate.id === dispatch.routingAttemptId);
            if (!attempt || !((x.tasks as TaskEntity[]).some(task => task.id === dispatch.taskId))
                || attempt.envelope.role !== 'implementer' || attempt.envelope.sliceId !== dispatch.taskId
                || attempt.envelope.planDigest !== binding.digest || attempt.envelope.executionDigest !== binding.executionDigest
                || !attempt.reservationRequestId || !dispatch.dispatchRequestId) return false;
            const acknowledgements = x.appliedRequests as Record<string, AppliedRequest>;
            const reservationAck = acknowledgements[attempt.reservationRequestId];
            const dispatchAck = acknowledgements[dispatch.dispatchRequestId];
            if (reservationAck?.outcome !== 'applied' || reservationAck.requestId !== attempt.reservationRequestId
                || reservationAck.resultRef !== attempt.id || dispatchAck?.outcome !== 'applied'
                || dispatchAck.requestId !== dispatch.dispatchRequestId || dispatchAck.resultRef !== dispatch.id) return false;
        }
    }
    if (x.routingIncidents !== undefined && (!Array.isArray(x.routingIncidents) || x.routingIncidents.length > 4096 || !x.routingIncidents.every(isWellFormedRoutingIncident))) return false;
...
function isWellFormedDispatch(x: unknown): x is DispatchRecord {
    return isObj(x) && Object.keys(x).every(key => ['id', 'taskId', 'at', 'routingAttemptId', 'dispatchRequestId'].includes(key))
        && typeof x.id === 'string' && x.id.length > 0 && typeof x.taskId === 'string' && x.taskId.length > 0
        && typeof x.at === 'string' && (x.routingAttemptId === undefined || (typeof x.routingAttemptId === 'string' && x.routingAttemptId.length > 0))
        && (x.dispatchRequestId === undefined || (typeof x.dispatchRequestId === 'string' && x.dispatchRequestId.length > 0));
}
...
function isWellFormedRoutingIncident(x: unknown): x is RoutingIncident {
    ...
    return x.schema === 'routing-incident/v1' && typeof x.id === 'string' && /^[a-f0-9]{24}$/.test(x.id)
        && typeof x.runStartedAt === 'string' && Number.isFinite(Date.parse(x.runStartedAt))
        && typeof x.target === 'string' && x.target.length > 0 && x.target.length <= 128
        && isRoutingSelection(x.selection) && typeof x.reasonCode === 'string' && /^[A-Z0-9_]{3,64}$/.test(x.reasonCode)
        && typeof x.firstAt === 'string' && Number.isFinite(Date.parse(x.firstAt))
        && typeof x.lastAt === 'string' && Number.isFinite(Date.parse(x.lastAt))
        && Number.isSafeInteger(x.affectedObligations) && (x.affectedObligations as number) > 0
        && Number.isSafeInteger(x.fallbackCount) && (x.fallbackCount as number) >= 0
        && Number.isSafeInteger(x.blockedCount) && (x.blockedCount as number) >= 0
        && ['pending', 'reported'].includes(String(x.alertState));
}
...
export function isRoutingEnvelope(x: unknown): x is RoutingEnvelopeRecord { if (!isObj(x) || !Object.keys(x).every(key => ['schema','runtime','role','planDigest','executionDigest','sliceId','requestedProfile','effectiveProfile','resolved','policyDigest','capabilityDigest','outcome','unavailableEvidence','nativeAgentType'].includes(key))) return false; const runtime = x.runtime; return x.schema === 'routing-envelope/v1' && isObj(runtime) && Object.keys(runtime).every(key => ['target','kind','version','accountScopeDigest'].includes(key)) && ['codex','claude-code','cursor','copilot','opencode','antigravity'].includes(String(runtime.target)) && typeof runtime.kind === 'string' && runtime.kind.length > 0 && runtime.kind.length <= 128 && typeof runtime.version === 'string' && runtime.version.length > 0 && runtime.version.length <= 128 && isDigest(runtime.accountScopeDigest) && typeof x.role === 'string' && ROUTING_ROLES.includes(x.role as typeof ROUTING_ROLES[number]) && isDigest(x.planDigest) && isDigest(x.executionDigest) && (x.sliceId === undefined || (typeof x.sliceId === 'string' && x.sliceId.length > 0 && x.sliceId.length <= 128)) && ['mechanical','integration','judgment','full'].includes(String(x.requestedProfile)) && ['mechanical','integration','judgment','full'].includes(String(x.effectiveProfile)) && isRoutingSelection(x.resolved) && isDigest(x.policyDigest) && isDigest(x.capabilityDigest) && ['native','degraded'].includes(String(x.outcome)) && Array.isArray(x.unavailableEvidence) && x.unavailableEvidence.length <= UNAVAILABLE_EVIDENCE.length && x.unavailableEvidence.every(value => typeof value === 'string' && UNAVAILABLE_EVIDENCE.includes(value as typeof UNAVAILABLE_EVIDENCE[number])) && (x.nativeAgentType === undefined || (runtime.target === 'claude-code' && typeof x.nativeAgentType === 'string' && /^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$/.test(x.nativeAgentType))); }
function isWellFormedImplementationLineage(x: unknown): x is ImplementationLineage { return isObj(x) && Object.keys(x).every(key => ['id','obligationId','sliceId','planDigest','executionDigest','initialProfile','initialEffort','attempts'].includes(key)) && typeof x.id === 'string' && x.id.length > 0 && typeof x.obligationId === 'string' && x.obligationId.length > 0 && typeof x.sliceId === 'string' && x.sliceId.length > 0 && isDigest(x.planDigest) && isDigest(x.executionDigest) && ['mechanical','integration','judgment'].includes(String(x.initialProfile)) && ['medium','high','runtime-default'].includes(String(x.initialEffort)) && Number.isSafeInteger(x.attempts) && (x.attempts as number) >= 0 && (x.attempts as number) <= 3; }
