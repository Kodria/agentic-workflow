...
export interface RoutingAttempt { schema: 'routing-attempt/v1'; id: string; obligationId: string; lineageId: string; attempt: number; envelope: RoutingEnvelopeRecord; envelopeDigest: string; fingerprint: string; state: 'reserved' | 'active' | 'complete' | 'blocked' | 'unknown'; nativeAgentId?: string; observed?: RoutingSelection; verdict?: 'pass' | 'fail' | 'inconclusive'; reasonCode?: string; administrativeRepair?: true; }
export interface ImplementationLineage { id: string; obligationId: string; sliceId: string; planDigest: string; executionDigest: string; initialProfile: 'mechanical' | 'integration' | 'judgment'; initialEffort: 'medium' | 'high'; attempts: number; }
...
function isWellFormedRoutingAttempt(x: unknown): x is RoutingAttempt {
    return isObj(x) && Object.keys(x).every(key => ['schema', 'id', 'obligationId', 'lineageId', 'attempt', 'envelope', 'envelopeDigest', 'fingerprint', 'state', 'nativeAgentId', 'observed', 'verdict', 'reasonCode', 'administrativeRepair'].includes(key))
