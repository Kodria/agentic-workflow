...
describe('exec-wrapper', () => {
    ...
    test.each([
        { argv: ['node'], cwd: '../escape' },
        { argv: [undefined], cwd: '.' },
        { argv: ['node', 'sentinel\u0000'], cwd: '.' },
    ])('rejects invalid execution inputs before sidecar writes (%#)', async inputs => {
        await expect(runExecWrapper({ logsRoot: path.join(dir, 'unused'), jobId: 'job', nonce: 'nonce', repoRoot: dir, ...inputs } as any)).rejects.toThrow();
        expect(fs.readdirSync(dir)).toEqual([]);
    });
