...
describe('captureCycleEvidence', () => {
  test('derives counts and opaque signatures without journal prose or identities', () => {
    const evidence = captureCycleEvidence({
      ...
      journal: {
        ...
        verdicts: [{ result: 'fail', detail: 'Alice saw secret prompt', receivedAt: '2026-08-22T10:00:05.000Z' }],
    ...
    expect(evidence).toMatchObject({
      ...
      qa: { findings: 1, fixes: 1 }, gates: { required: 1, firstEvaluationsPassed: 1, firstPass: true },
      pr: { provider: 'github', number: 12 },
    });
  ...
  test('keeps the cycle identity stable across host paths', () => {
    const source = {
    ...
    const first = captureCycleEvidence({ ...source, root: '/host-a/work/repo' });
    const second = captureCycleEvidence({ ...source, root: '/host-b/elsewhere/repo' });
