...
- **Process finding — R1a's harness-retro archive did not take effect, root cause found and fixed:** the active ledger read at the start of this retro spanned 2026-08-23 through 2026-08-24, i.e. it carried R1a's own findings (already fixed and merged) alongside R1b's new ones. `.awm/ledger/archive/` held only one file, dated before R1a started — meaning R1a's own harness-retro (same branch, prior cycle, reported complete) ran `awm ledger archive` but the active ledger was never actually cleared. This is the **third** occurrence of this exact gap: the 2026-08-22 retro log already flagged an R1→R2 archive skip and explicitly predicted a third instance would warrant a process fix. Chasing it down this time (rather than just adding a detection step) surfaced the likely root cause: `awm evidence capture` (`cli/src/commands/evidence/index.ts:runEvidenceCapture`), which Step 11 ran immediately before archiving and gated the archive on succeeding, requires `.awm/journal/<branch>/state.json` unconditionally — it treats a MISSING journal identically to a CORRUPT one, with no fallback. Journal-first mode is explicitly opt-in (`subagent-driven-development`'s own SKILL.md), and this branch never initialized it (`ls .awm/journal/` → does not exist). Reproduced live: `node cli/dist/src/index.js evidence capture --plan docs/plans/2026-08-23-process-lifecycle-r1b-plan.md` fails with `current journal is unavailable or corrupt` on this exact repo/branch right now — meaning R1a's harness-retro almost certainly hit the identical wall and, instead of correctly stopping per its own "a failed capture stops the retro and does not archive the ledger" rule, proceeded anyway. A second, independent bug in the same step compounded this: Step 11's `active_plan` resolver reused `development-process`'s SessionStart re-anchor resolver (open checkboxes, no `awm-qa-complete`) — the opposite of what a plan being retro'd looks like — and with multiple plans in `docs/plans/`, it silently resolved to an unrelated stale plan instead of R1b's (confirmed by running the exact script). Both cured in the registry repo's `skills/harness-retro/SKILL.md` working copy (`awm-baseline-registry`, bundle `dev` 3.5.0→3.5.3, three chained patches): (1) the resolver now looks for `awm-qa-complete` present / `awm-retro-complete` absent; (2) capture is now conditional on `.awm/journal/` actually existing, skipping gracefully (not failing) when it doesn't; (3) a post-archive `awm ledger list` must report empty before the retro adds its completion marker, so a future recurrence surfaces immediately instead of silently compounding into the next cycle.
...
## 2026-09-05 — Issue #93: cancelación durable de cohortes

- **Evidencia:** QA independiente encontró y el ciclo corrigió la invalidez de evidencia final, una frontera por marcador, convergencia request→teardown, fail-closed de Git, liberación de `integration.lock` y rechazo de cohortes no reconciliables. Verificación final: Jest 282 suites aprobadas (3515 tests; 2 suites/16 tests omitidos) y `awm sensors run` con `overall: pass`.
- **Triage desatendido:** los hallazgos ya quedan curados como pruebas de regresión específicas; no se agrega una regla nueva de `AGENTS.md`/`CONSTITUTION.md` ni un sensor, porque serían mecanismos nuevos fuera de la autoridad de este issue. La cobertura empírica fue `partial`/`inconclusive` por detectores no instalados; queda como recomendación de plataforma, no como cambio de #93.
- **Descarte documentado:** el ciclo de dependencia `tracks ↔ teardown-driver` es baseline anterior a #93 y no se altera dentro de este PR. El ledger se conserva para auditoría según el precedente del repo; no se archiva al no existir journal-first verificable.

## 2026-08-26 — R4a compact plan/currentness: cierre documental y retro desatendido

- **Evidencia del ciclo:** cuatro slices (S1–S4) cerraron sus revisiones de especificación y calidad; el ledger activo contiene 69 entradas (62 hallazgos, 7 victorias). La evidencia de entrega reportada para el HEAD candidato es Jest completo: 269 aprobados / 2 omitidos; sensores: `overall: pass`. La documentación de usuario ya fue contrastada con el binario y su contrato queda cubierto por `cli/tests/structural/r4-plan-currentness-docs.test.ts`; esta fase no encontró una superficie adicional que documentar.
- **Triage — bypasses múltiples de S1:** `awm ledger recurring --min 2` agrupa 45 hallazgos convergentes de validación de `compact-slices/v1` (esquemas escapados/futuros, lanzadores e intérpretes, rutas Windows, límites, Markdown y trazabilidad). No son recurrencias entre ciclos: son una exploración adversarial concentrada de un mismo borde, corregida antes del cierre y respaldada por la matriz comportamental de `cli/tests/core/plan/validate.test.ts`. Se descartan como reglas individuales: añadir una regla de contexto por cada variante duplicaría el contrato fail-closed ya presente y no aumentaría detección.
- **Recomendación no aplicada:** el volumen y la diversidad de esos bypasses justifican que el propietario de diseño evalúe, en un ciclo autorizado, consolidar un corpus de mutación/adversarial reutilizable para fronteras de parser y argv. No se añade aquí: sería una nueva política/mecanismo de test, no una cura autorizada de este cierre.
- **Triage — preflight e2e:** `r4-s4-preflight-update-side-effect` se corrigió con `cli/tests/integration/preflight-json-pipe.e2e.test.ts`, que prueba que `preflight`, JSON y `--verify-sensors --json` no inician el worker pasivo ni escriben la caché; el mismo archivo conserva JSON parseable y ausencia de escrituras ante un sensor v2 que sale con código 2. Se descarta otra regla: la regresión e2e específica ya cubre el contrato. Recomendación no aplicada: si se amplía la superficie de `preflight`, mantener estos casos como smoke tests de proceso real, porque los unit tests de composición no observan workers diferidos ni contaminación del pipe JSON.
- **Descartes restantes (modo desatendido):** los hallazgos S2–S4 de límites, redacción de secretos, transporte, semver, inventario y documentación son instancias corregidas con pruebas de regresión del slice; las 7 victorias confirman defensas ya materializadas. No hay una nueva regla autorizada ni se modifica `AGENTS.md` o `CONSTITUTION.md`. No se registran tokens, caché, modelo ni costos: no hubo observación atribuible en este cierre.
- **Ledger y desviación:** no existe `.awm/journal/` en este ciclo; por tanto no se fabrica una captura `journal-first`. Además, el gate de compatibilidad previo al archive no puede leer un `minCliVersion` válido: este worktree no contiene `awm-registry.json` (ni en la raíz ni en `cli/`). No se ejecuta `awm ledger archive` ni se borra/rota evidencia; el ledger activo de 69 entradas queda íntegro para auditoría. El marker se añade por el mandato explícito de cierre desatendido, dejando esta desviación visible en vez de simular compatibilidad o destruir historial.
## 2026-09-07 — Una suite Jest sin fixture reintroducía el pin global del baseline

- **Clase:** lógica / aislamiento de pruebas.
- **Ocurrencias:** síntoma recurrente en la máquina; una causa exacta confirmada. El segundo `describe` de `cli/tests/commands/pin.test.ts` quedó fuera de los hooks que aislaban `HOME` y `AWM_HOME`, y su último caso escribía literalmente `baseline: 2.0.0` en las preferencias reales.
- **Regla:** `cli/jest.global-setup.js` asigna un `AWM_HOME` propiedad de la suite para todos los workers; `cli/tests/structural/jest-environment-is-isolated.test.ts` exige esa frontera; los hooks de `pin.test.ts` ahora cubren todos sus casos.
- **Sensor:** structural-test + suite Jest completa. La regresión se observó roja antes del cambio (`AWM_HOME` era `undefined`) y verde después; 284 suites / 3.545 pruebas activas aprobaron. El SHA-256 de `~/.awm/preferences.json` fue idéntico antes y después de la suite completa.
...
## 2026-09-24 — Matriz nativa de modelos por máquina

- **Evidencia:** ocho entradas del ledger, siete hallazgos distintos y ningún win registrado. `ledger recurring --min 2` agrupó cuatro entradas de lógica como convergentes, pero corresponden a tres fronteras distintas (esfuerzo Claude, fallback de linaje y reporte); no se tratan como una sola causa repetida. Seis hallazgos se corrigieron con regresiones focalizadas y una revisión independiente; el séptimo, custody por deriva global, se descartó como defecto tras comprobar que la misma deriva invalida también `fullCapability` y el controller.
- **Curas ejecutables:** las pruebas cubren reserva Claude runtime-default, escalada después de fallo nativo, proof posterior a reserva y alcance actual, lectura JSON acotada, reporte separado de selección/modelo/uso y diagnóstico pasivo de cobertura local. Una prueba roja-verde verificó el caso de cobertura pasiva. No se añade una regla global: cada fallo tiene su guard conductual junto al código que lo decide.
- **Verificación:** 330 suites y 4.208 pruebas PASS; 2 suites y 16 pruebas omitidas. Build y sensores habilitados (`lint`, `typecheck`, `depcheck`) dieron `overall: pass`. `sensors coverage --json` reportó `overall: gaps`, `static: gaps` y `empirical: partial`, con nueve ocurrencias aún sin clase empírica; no equivale a certificar sensores opcionales ni a medir ahorro de tokens.
...
## 2026-09-24 — Handoff supervisado de S1 tras reapertura de #196

- **Hallazgos:** 20 entradas de QA, ningún win registrado. Seis grupos convergentes señalan huecos de prueba integral, admisión de reemplazo, prueba nativa, ACK exacto y mensaje de salida. Los hallazgos se corrigieron con regresiones locales; la convergencia del ledger no demuestra una regla global que corresponda imponer a otros proyectos.
- **Curas ejecutables:** `reopened-196.test.ts` construye el CLI actual, prueba ACK de reserva y dispatch, negativos de vínculo v2, RED después de admisión, identidad del controlador, SIGINT y no duplicación de S1. Las fixtures de `supervisor-loop` y `plan-lifecycle` adoptan explícitamente la identidad simulada; el supervisor conserva los casos legítimos de track sin generación local y finalización canónica con controlador pausado.
- **Verificación:** tras acotar la excepción del controlador pausado, la suite global final pasó 333 suites y 4.269 tests; 2 suites y 16 tests omitidos por sus condiciones existentes. Build y sensores `lint`, `typecheck`, `depcheck` dieron `overall: pass`. No se afirma un despacho pagado a Codex.
- **Cobertura:** `awm sensors coverage --json` reportó `overall: gaps` por detectores opcionales faltantes y `empirical: partial` con las 20 entradas sin clase empírica. No se altera el pack de sensores como efecto lateral de #196.
- **Recomendaciones no aplicadas:** considerar en un ciclo dedicado un harness reutilizable que pruebe la secuencia CLI supervisor → ACK durable → child nativo simulado → observación, y clasificar los hallazgos empíricos. Requieren decisión de alcance; no se modifican reglas globales ni el Context Kernel.
- **Descartes:** no se añaden reglas a `AGENTS.md`, `CONSTITUTION.md` ni sensores nuevos: los invariantes de esta corrección ya tienen pruebas cercanas al código. La simulación local no se presenta como certificación de un proveedor real.
- **Captura de ciclo:** `awm watch journal-status --json` devolvió `missing` para esta rama. Se omite `awm evidence capture`; la evidencia manual es el plan, los tests, los sensores y QA. No se declara `COMPLETE` de un ciclo desatendido inexistente.

## 2026-09-25 — Recuperación de runtime y dispatch histórico de #196

- **Hallazgos:** siete entradas de QA, sin wins; dos convergen en identidad de la generación para reutilizar sensores. Se clasifican como lógica/custodia y seguridad de autorización de despacho; las curas están en regresiones conductuales del CLI, sin regla global nueva.
- **Curas verificadas:** el sensor PASS se reutiliza solo con identidad viva y vinculada por nonce/digest; un reemplazo exige sensores frescos. Una consulta de runtime inconclusa conserva causa recuperable sin llamarla cambio medido. La recuperación offline rehace los gates. El handoff histórico exige el digest exacto de un ACK aplicado anterior y uno nuevo, enlazado a la reserva, antes de cualquier hijo; se rechazan reintentos con propiedad nativa incierta.
- **Verificación:** 134 tests focales PASS, prueba integrada adicional contra CLI recompilado PASS y sensores habilitados `lint`, `typecheck`, `depcheck` `overall: pass`. Admisión estricta del plan `admitted`. El smoke con el journal privado y un hijo Codex real sigue pendiente fuera de esta rama; no se declara #196 cerrado.
- **Cobertura y descartes:** `sensors coverage --json` sigue en `overall: gaps`, `empirical: partial` por detectores opcionales y hallazgos sin `defectClass`; no se instala un sensor ni se cambia AGENTS/CONSTITUTION por este issue. Los siete hallazgos ya tienen pruebas locales o corrección específica; una regla transversal nueva requiere alcance propio.
- **Captura:** `awm watch journal-status --json` devolvió `missing`; no se inventa `COMPLETE` ni se ejecuta captura de ciclo. Evidencia manual durable: plan de esta rama, tests de regresión, sensores y revisión independiente.
- **Corrección antes de integrar:** al contrastar el CLI instalado 9.12.2 se detectó que `job register` añade `entity` después del JSON del llamador, y el digest conserva el orden de claves. La primera fixture había usado otro orden y rechazaba un ACK histórico genuino. La regresión reprodujo RED con el digest real de 9.12.2; la cura admite solo las seis permutaciones de `entity`, `dispatchId` y `taskId`, sin campos extra. El conjunto afectado pasó 135/135 y una revisión independiente no halló defectos críticos/importantes. No se añade una regla global: el invariante está fijado en la prueba conductual del dispatch.
