...
via its own config (`stryker.conf.json`) to keep run times manageable.

Once `enabled: true`, mutation runs on every `awm sensors run` (the default gate).
Use `awm sensors run --slow` to run only the slow sensors (mutation + semgrep) and
skip the fast ones (typecheck, lint, test).
