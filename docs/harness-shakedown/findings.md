...
- **CONFIRMADO en la corrida de OpenCode (2026-06-05):** OpenCode siguió el flujo (development-process→brainstorming→plan→exec) improvisando desde el contexto inyectado, PERO con fidelidad menor: design y plan **sin commitear** (untracked), 1 commit grueso vs 5 commits granulares de Claude, año mal en filenames (2025), docs más finos, sin las dos etapas de review. **Veredicto por capas:** (1) entrega de contexto = agnóstica ✅; (2) seguir el flujo = funciona en ambas ✅; (3) fidelidad/enforcement de ejecución = NO agnóstica ⚠️ (Claude enforce vía Skill/Agent tooling + hook de sensores; OpenCode aproxima). La garantía depende de la disciplina del agente → para hacerla agnóstica, mover enforcement a gates determinísticos (ver ⭐ INSIGHT CENTRAL).
- **Relación:** consecuencia de [Hallazgo #2]. Fix de #2 (gate real con tsc/lint/test) mitiga este también.
- **Estado:** CONFIRMADO — la fidelidad de ejecución no es agnóstica hoy.
