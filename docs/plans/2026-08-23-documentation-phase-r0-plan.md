...
- [x] **CA-0.2** — la fase corre en modo desatendido de punta a punta, sin pausas, en un ciclo real. Verificado por observación directa de esta misma ejecución: las 10 tasks + el fix loop de `post-implementation-qa` corrieron sin una sola pausa de confirmación al usuario, exactamente como exige `**Modo de ejecución:** desatendido` en el header de este plan.
- [x] **CA-0.3** — con el registry de documentación ausente (que es el estado por defecto: es opt-in), la fase corre igual y la rama cierra. Dos formas de evidencia, distinguidas con precisión: (a) el contrato del skill `post-implementation-docs` está testeado — `tests/r10-documentation-phase-contract.test.mjs` afirma que el skill nombra `awm-documentation-registry` y declara explícitamente "nunca bloquea"; (b) evidencia operacional real de este mismo ciclo: el entorno de esta sesión genuinamente no tiene el registry de documentación instalado, y la Task 10 (el trabajo de documentación de este mismo plan) completó siguiendo la misma disciplina sin necesitarlo. **Precisión honesta:** (b) no es una invocación en vivo del skill `post-implementation-docs` a través de `development-process` — Task 10 se despachó como subagente siguiendo los pasos del plan, no cargando el skill real vía el harness de AWM. La garantía de comportamiento en producción descansa en (a).
- [ ] **Entrega ordenada** — el contenido se taggea y publica **antes** de que el CLI mergee a `main`. Si el CLI llegara primero, los usuarios verían planes en `docs_pending` sin tener todavía el skill que produce el marker. **No cerrado por diseño:** el tag lo corta `auto-tag.yml` automáticamente al mergear a `main` (ver CONSTITUTION.md, "Release de contenido"); ninguno de los dos repos está mergeado todavía, así que esta condición estructuralmente no puede ser verdadera hasta ese momento — corresponde a `finishing-a-development-branch`, no a esta fase de QA.
