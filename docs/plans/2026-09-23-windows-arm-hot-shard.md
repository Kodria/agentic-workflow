...
CI run `35808870550` passed all seven jobs. Its slowest job, the Windows ARM remainder, took 16m27s (02:04:12–02:20:39 UTC), down 4m26s (21.2%) from the 20m53s baseline in run `35798322319`. This measurement applies to the first PR commit. The subsequent QA amendment narrowed the ignore regex in both workflows without changing the current suite partition. Run `35811474914` exposed an LF-only assumption in the structural test on Windows x64; that test now covers CRLF and the final revision requires a fresh seven-job CI pass.
