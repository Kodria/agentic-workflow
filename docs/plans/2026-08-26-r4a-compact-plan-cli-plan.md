...
7. Prove traceability: every requirement is unique and owned by exactly one slice; every command/source/slice reference resolves; no requirement, source, or command is orphaned; dependencies are unique, non-self-referential, and acyclic; every slice declares exactly `specification` and `code-quality` review evidence, a `bounded` or `full-context` risk, and at least one nonempty fallback condition.
