# Story Mapping Skill Improvement — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Rewrite the story-mapping skill to correctly follow Jeff Patton's methodology — 4-level hierarchy, validation tests, decision tree, execution phases, prioritization guide, and validation checklist.

**Architecture:** Two markdown files to rewrite. SKILL.md gets a complete rewrite maintaining the same operational structure (3 modes, step 0, termination) but with correct methodology. Template gets updated to reflect the 4-level hierarchy.

**Tech Stack:** Markdown only. No code changes.

**Design doc:** `docs/plans/2026-03-31-story-mapping-improvement-design.md`

---

### Task 1: Rewrite SKILL.md — Frontmatter + intro + methodology foundation

**Files:**
- Modify: `registry/skills/story-mapping/SKILL.md`

**Step 1: Replace the entire SKILL.md with the new version**

The new SKILL.md must contain these sections in this order. Write the **frontmatter**, **intro**, **methodology foundation** (4 levels + tests + decision tree + non-action concepts), and **Step 0 autodiscovery** (unchanged from current).
...
Run: `head -5 registry/skills/story-mapping/SKILL.md`
Expected: The YAML frontmatter opening `---` and name/description fields.

**Step 3: Commit**

```bash
git add registry/skills/story-mapping/SKILL.md
git commit -m "refactor: rewrite story-mapping SKILL.md foundation — 4-level hierarchy, validation tests, decision tree"
```

---

### Task 2: Add Paso 0 + Paso 1 + Modo A to SKILL.md

**Files:**
- Modify: `registry/skills/story-mapping/SKILL.md` (append after the foundation from Task 1)

**Step 1: Append Paso 0 (autodiscovery), Paso 1 (mode detection), and Modo A (generate)**

Append the following sections after the "Detección proactiva de errores comunes" section:
...
Run: `grep -c "^## " registry/skills/story-mapping/SKILL.md`
Expected: Count of H2 sections should be growing (verify sections are appended correctly).

**Step 3: Commit**

```bash
git add registry/skills/story-mapping/SKILL.md
git commit -m "feat: add Paso 0, mode detection, and Modo A with 3 confirmation gates"
```

---

### Task 3: Add Modo B (6 phases) to SKILL.md

**Files:**
- Modify: `registry/skills/story-mapping/SKILL.md` (append after Modo A)

**Step 1: Append Modo B with 6 execution phases**

Append after the Modo A section:
...
Tras completar: *"Todas las Activities descompuestas: {n} Tasks en total. ¿Pasamos a las historias de usuario? Puedes describirlas en cualquier orden — yo las ubico en el lugar correcto."*
...
Ejecuta la **Checklist de Validación** (ver sección dedicada) sobre el mapa completo y reporta resultados al grupo.
...
Run: `grep -c "^### Fase" registry/skills/story-mapping/SKILL.md`
Expected: `6` (Fase 0 through Fase 5)

**Step 3: Commit**

```bash
git add registry/skills/story-mapping/SKILL.md
git commit -m "feat: add Modo B with 6 execution phases aligned to Jeff Patton methodology"
```

---

### Task 4: Add Modo C + Rules + Checklist + Termination to SKILL.md

**Files:**
- Modify: `registry/skills/story-mapping/SKILL.md` (append after Modo B)

**Step 1: Append Modo C, transversal rules, checklist, and termination**

Append after the Modo B cierre section:
...
1. **Ejecuta la Checklist de Validación** y reporta advertencias
...
**Step 2: Verify the complete SKILL.md structure**

Run: `grep "^## " registry/skills/story-mapping/SKILL.md`
Expected output (all H2 sections in order):
```
## Fundamento metodológico — Los 4 niveles de Jeff Patton
## El eje horizontal: flujo narrativo, NO secuencia estricta
## Árbol de decisión para elementos ambiguos
...
```bash
git add registry/skills/story-mapping/SKILL.md
git commit -m "feat: add Modo C, transversal rules, validation checklist, and termination phase"
```

---

### Task 5: Update story-map-template.md to 4-level structure

**Files:**
- Modify: `registry/skills/template-wizard/resources/templates/story-map-template.md`
...
Run: `grep "^## \|^### \|^####" registry/skills/template-wizard/resources/templates/story-map-template.md`
Expected:
```
## Goal
## Personas
...
git commit -m "feat: update story-map template to 4-level hierarchy (Goal, Activity, Task, Story)"
```
...
**Files:**
- Read: `registry/skills/story-mapping/SKILL.md`
- Read: `registry/skills/template-wizard/resources/templates/story-map-template.md`

**Step 1: Verify SKILL.md has no references to old terminology**

Run: `grep -in "step:" registry/skills/story-mapping/SKILL.md`
Expected: No matches (all "Step:" references should now be "Task:")

Run: `grep -in "cronológico" registry/skills/story-mapping/SKILL.md`
Expected: No matches (replaced with "narrativo")

Run: `grep -in "🟪" registry/skills/story-mapping/SKILL.md`
Expected: No matches (replaced with 🟡)

**Step 2: Verify template has no old terminology**

Run: `grep -in "Step:" registry/skills/template-wizard/resources/templates/story-map-template.md`
Expected: No matches

Run: `grep -in "🟪" registry/skills/template-wizard/resources/templates/story-map-template.md`
Expected: No matches

**Step 3: Verify SKILL.md completeness — all 13 H2 sections present**

Run: `grep "^## " registry/skills/story-mapping/SKILL.md`
Expected: 13 H2 sections as listed in Task 4, Step 2.

**Step 4: Count total lines to verify completeness**

Run: `wc -l registry/skills/story-mapping/SKILL.md`
Expected: ~350-450 lines (the current file is 191 lines, so roughly 2x growth is expected).
